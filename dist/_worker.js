...
async function handleApi(request, env2, url) {
  ...
  try {
    await env2.PORTALS_DB.exec(`CREATE TABLE IF NOT EXISTS portals (
    ...
    const id = pathParts.length >= 3 && pathParts[pathParts.length - 1] !== "portals" ? Number(pathParts[pathParts.length - 1]) : null;
    ...
    switch (request.method) {
      case "GET": {
        if (id) {
          const portal = await env2.PORTALS_DB.prepare("SELECT * FROM portals WHERE id = ?").bind(id).first();
          if (!portal) return new Response("Not found", { status: 404 });
          portal.password = await decryptField(portal.password_enc, env2.PORTALS_ENCRYPTION_KEY);
          delete portal.password_enc;
          return Response.json(portal, { headers: corsHeaders });
        }
        let { results } = await env2.PORTALS_DB.prepare("SELECT * FROM portals ORDER BY type, name").all();
        if (results.length === 0) {
          for (const p of DEFAULT_PORTALS) {
            const enc = p.password ? await encrypt(p.password, env2.PORTALS_ENCRYPTION_KEY) : "";
            await env2.PORTALS_DB.prepare(
              "INSERT INTO portals (type, name, username, password_enc, url) VALUES (?, ?, ?, ?, ?)"
            ).bind(p.type, p.name, p.username, enc, p.url).run();
          }
          results = await env2.PORTALS_DB.prepare("SELECT * FROM portals ORDER BY type, name").all();
      ...
      case "POST": {
        ...
        const enc = body.password && env2.PORTALS_ENCRYPTION_KEY ? await encrypt(body.password, env2.PORTALS_ENCRYPTION_KEY) : "";
        const portal = await env2.PORTALS_DB.prepare(
          "INSERT INTO portals (type, name, username, password_enc, url) VALUES (?, ?, ?, ?, ?) RETURNING *"
        ).bind(body.type, body.name, body.username, enc, body.url || "").first();
      ...
      case "PUT": {
        ...
        if (body.password !== void 0) {
          enc = body.password && env2.PORTALS_ENCRYPTION_KEY ? await encrypt(body.password, env2.PORTALS_ENCRYPTION_KEY) : "";
        }
        let query = "UPDATE portals SET type = ?, name = ?, username = ?, url = ?, updated_at = datetime('now')";
        const params = [body.type, body.name, body.username, body.url || ""];
        if (enc !== void 0) {
          query += ", password_enc = ?";
          params.push(enc);
        }
        ...
        params.push(id);
        const portal = await env2.PORTALS_DB.prepare(query).bind(...params).first();
        if (!portal) return new Response("Not found", { status: 404 });
        portal.password = body.password ?? await decryptField(portal.password_enc, env2.PORTALS_ENCRYPTION_KEY);
        delete portal.password_enc;
        return Response.json(portal, { headers: corsHeaders });
      }
      case "DELETE": {
        if (!id) return new Response("Bad request", { status: 400 });
        const result = await env2.PORTALS_DB.prepare("DELETE FROM portals WHERE id = ?").bind(id).run();
...
async function decryptField(stored, key) {
...
async function decrypt(stored, keyHex) {
